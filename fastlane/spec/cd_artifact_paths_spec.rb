...
describe CDArtifactPaths do
  describe 'when specifying a stage' do
    ...
    it 'it specifies the derived data path in the build directoy' do
      expect(subject.derived_data_path).to eq('__cd_pipeline_artifacts__/derived_data')
    end

    it 'it specifies the result bundle in stage directoy' do
      expect(subject.result_bundle_path('testing_build')).to eq('__cd_pipeline_artifacts__/dev_stage/build_result/testing_build.xcresult')
    end

    it 'it specifies the output directory in stage directoy' do
      expect(subject.output_directory).to eq('__cd_pipeline_artifacts__/dev_stage/build_result')
    end
